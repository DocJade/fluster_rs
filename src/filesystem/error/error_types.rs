...
pub const INVALID_SEEK: c_int = libc::ESPIPE;
pub const STALE_HANDLE: c_int = libc::ESTALE; // Tried to use a filehandle that is stale. go get a new one.
pub const GENERIC_FAILURE: c_int = libc::EIO; // Generic IO error.
pub const FILE_TOO_BIG: c_int = libc::EFBIG;
pub const TRY_AGAIN: c_int = libc::ERESTART; // "interrupted system call should be restarted"
pub const BUSY: c_int = libc::EBUSY; // Device / filesystem is busy, try again later.
