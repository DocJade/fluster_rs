...
static NO_SUCH_ITEM: LazyLock<PosixError> = LazyLock::new(|| PosixError::new(2, "Yo, the containing directory does not exist."));
static FILE_NAME_TOO_LONG: LazyLock<PosixError> = LazyLock::new(|| PosixError::new(91, "Yo, names have to be at most 255 characters long."));
static FILE_ALREADY_EXISTS: LazyLock<PosixError> = LazyLock::new(|| PosixError::new(17, "Yo, that file already exists."));
static FILE_TOO_BIG: LazyLock<PosixError> = LazyLock::new(|| PosixError::new(27, "Yo, we cant handle such a large file."));
static NOT_SUPPORTED: LazyLock<PosixError> = LazyLock::new(|| PosixError::new(134, "Yo, whatever you're trying to do here, we dont do that."));
static ILLEGAL_SEEK: LazyLock<PosixError> = LazyLock::new(|| PosixError::new(29, "Yo, you cant seek that way bro."));
static NOT_A_DIRECTORY: LazyLock<PosixError> = LazyLock::new(|| PosixError::new(20, "Yo, this has to be called on a directory."));
...
fn spoofed_file_attributes(file_size: u64, creation_time: InodeTimestamp, modified_time: InodeTimestamp, item_type: FileKind) -> FileAttribute {
    // Convert the times
    let mut system_creation_time: SystemTime = SystemTime::UNIX_EPOCH;
    // Add the seconds
    system_creation_time = system_creation_time.checked_add(
        Duration::from_secs(creation_time.seconds)
    ...
    system_creation_time = system_creation_time.checked_add(
        Duration::from_nanos(creation_time.nanos as u64 * 1000) 
    ).expect("Time stuff sucks, but this should be fine.");

    let mut system_modified_time: SystemTime = SystemTime::UNIX_EPOCH;
    system_modified_time = system_modified_time.checked_add(
        Duration::from_secs(modified_time.seconds)
    ).expect("Time stuff sucks, but this should be fine.");
    system_modified_time = system_modified_time.checked_add(
        Duration::from_nanos(modified_time.nanos as u64 * 1000) 
    ).expect("Time stuff sucks, but this should be fine.");
...
impl FuseHandler<PathBuf> for FlusterFS {
    ...
    fn open(
        ...
        let file_name: String = file_id.file_name()
