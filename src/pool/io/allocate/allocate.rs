...
use crate::pool::{disk::{drive_struct::{DiskType, FloppyDrive, FloppyDriveError}, generic::{block::allocate::block_allocation::BlockAllocation, generic_structs::pointer_struct::DiskPointer}, standard_disk::standard_disk_struct::StandardDisk}, pool_actions::pool_struct::{Pool, GLOBAL_POOL}};
...
fn go_find_free_pool_blocks(blocks: u16) -> Result<Vec<DiskPointer>, FloppyDriveError> {
    ...
    let probable_disk = GLOBAL_POOL.get().expect("Single threaded").try_lock().expect("Single threaded").header.disk_with_next_free_block;
    let highest_disk = GLOBAL_POOL.get().expect("Single threaded").try_lock().expect("Single threaded").header.highest_known_disk;
    ...
    loop {
        ...
        match disk.find_free_blocks(blocks - free_blocks.len() as u16) {
            ...
            Err(amount) => {
                ...
                let blockie_doos = disk.find_free_blocks(amount).expect("We already asked how much room you had.");
