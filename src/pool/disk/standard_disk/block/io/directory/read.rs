...
impl DirectoryBlock {
    ...
    /// May swap disks.
    /// 
    /// Optionally returns to a the start disk when done.
    pub fn contains_item(&self, item_to_find: &NamedItem, start_disk: u16) -> Result<Option<DirectoryItem>, FloppyDriveError> {
        ...
        // Get items
        let items: Vec<DirectoryItem> = self.list(start_disk)?;
    ...
    pub fn list(&self, start_disk: u16) -> Result<Vec<DirectoryItem>, FloppyDriveError> {
        go_list_directory(self, start_disk)
    }
}

// Functions

fn go_list_directory(block: &DirectoryBlock, start_disk: u16) -> Result<Vec<DirectoryItem>, FloppyDriveError> {
    ...
    let mut current_disk: u16 = start_disk;
    ...
    // Return to the start disk.
    _ = FloppyDrive::open(start_disk)?;
